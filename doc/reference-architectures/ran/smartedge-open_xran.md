...
```
<!-- omit in toc -->
# O-RAN Front Haul Sample Application in Smart Edge Open 
...
- [xRAN Sample App Deployment in Smart Edge Open](#xran-sample-app-deployment-in-smart-edge-open)
  - [Hardware Configuration and Checks](#hardware-configuration-and-checks)
  - [Deploy xRAN sample app O-DU and O-RU in Smart Edge Open Network Edge](#deploy-xran-sample-app-o-du-and-o-ru-in-smart-edge-open-network-edge)
    ...
    - [Deploy Smart Edge Open NE](#deploy-smartedge-open-ne)
...
![Intel X710 NIC Pipeline](smartedge-open_xran_images/xran_img1.png)
...
![Advanced classification and lookup on XL710](smartedge-open_xran_images/xran_img2.png)
...
![O-RAN Font Haul Functional Split 7.2x](smartedge-open_xran_images/xran_img3.png)
...
![O-DU - O-RU Data Flows](smartedge-open_xran_images/xran_img4.png)
...
![DL C-Plane and DL U-Plane Package Exchange](smartedge-open_xran_images/xran_img6.png)
...
![Native Ethernet Frame with VLAN](smartedge-open_xran_images/xran_img7.png)

Native IPv4 Packet with VLAN:

![Native IPv4 Packet with VLAN](smartedge-open_xran_images/xran_img8.png)

eCPRI transport header:

![eCPRI Transport Header](smartedge-open_xran_images/xran_img9.png)
...
![Radio Application Header](smartedge-open_xran_images/xran_img18.png)
...
![Section Application Header](smartedge-open_xran_images/xran_img11.png)

Data Section Payload Format:

![Section Payload](smartedge-open_xran_images/xran_img12.png)
...
![Radio Application Common Header of a C-Plane Packet Section Type 1](smartedge-open_xran_images/xran_img19.png)
...
![Section Application Header for C-Plane Packet Section Type 1](smartedge-open_xran_images/xran_img13.png)
...
![Precision Time Protocol Packet Format](smartedge-open_xran_images/xran_img14.png)
...
![Supported Data Flows](smartedge-open_xran_images/xran_img20.png)
...
![5GNR L1app Threads](smartedge-open_xran_images/xran_img16.png)
...
![xRAN Library Sublayers](smartedge-open_xran_images/xran_img22.png)
...
![xRAN Sample Application](smartedge-open_xran_images/xran_img17.png)
...
![software vs hardware timestamping](smartedge-open_xran_images/xran_img25.png)
...
![eCPRI Profile PCTYPES](smartedge-open_xran_images/xran_img21.png)


# xRAN Sample App Deployment in Smart Edge Open
As Smart Edge Open NE is based on Kubernetes architecture, the xRAN sample application deployed on Smart Edge Open platforms is expected to work in a pod environment. As of the Smart Edge Open release 20.06, pods with O-DU and O-RU applications are placed on the same K8s node. Thus, for this scenario, PTP synchronization between O-DU and O-RU is not needed. O-DU and O-RU can be deployed in one or two separate pods. Nevertheless, the default configuration of the xRAN application in Smart Edge Open is set to deploy them in separate pods. O-DU and O-RU are expected to communicate through an Ethernet connection. The machine hosting the two pods must have one Intel® Ethernet Controller X710 NIC with two ports connected via ETH. The machine must be SRIOV-enabled as two VFs must be created on each of the physical ports and made available to each application pod.

Deployment of O-DU and O-RU applications on separate K8s nodes will be enabled in a future Smart Edge Open release. This scenario requires the two nodes to be synchronized according to the Precision Time Protocol (PTP). PTP synchronization of nodes is enabled in Smart Edge Open 20.06. The instructions are provided [here](#ptp-synchronization).
...
![PTP configuration C1](smartedge-open_xran_images/xran_img23.png)


2. O-DU and O-RU are synchronized to another primary clock available in the network:

![PTP configuration C2](smartedge-open_xran_images/xran_img24.png)
...
## Deploy xRAN sample app O-DU and O-RU in Smart Edge Open Network Edge

Before starting the deployment script, Smart Edge Open should be configured according to the instructions available [here](../../getting-started/smartedge-open-cluster-setup.md)
...
Detailed instructions on configuring SRIOV for Smart Edge Open can be found [here](../../building-blocks/enhanced-platform-awareness/smartedge-open-sriov-multiple-interfaces.md)
...
1. For PTP Configuration 1 [see](#xran-sample-app-deployment-in-smartedge-open)
...
2. For PTP Configuration 3 [see](#xran-sample-app-deployment-in-smartedge-open)
...
At this stage, Smart Edge Open with enabled SRIOV should be deployed. It is also expected that DPDK version 19.11 is installed on the node, and `vfio-pci` driver is loaded according to instructions given [here](https://cdrdv2.intel.com/v1/dl/getContent/611268).
