...
1. Select the flavor for deployment of CollectD from the OEK during OpenNESS deployment - flavor is to be selected with `telemetry_flavor: <flavor name>`.
   ...
   In the event of using the `flexran` profile, `n3000-1-3-5-beta-cfg-2x2x25g-setup.zip` and `n3000-1-3-5-beta-rte-setup.zip` need to be available in `./openness-experience-kits/opae_fpga` directory; for more information about the packages see [FPGA support in OpenNESS](https://github.com/open-ness/ido-specs/blob/master/doc/enhanced-platform-awareness/openness-fpga.md#edge-controller)
