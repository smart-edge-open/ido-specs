...
1. On the traffic generating host: 
* in `tx_video.sh` in the directory containing the image Dockerfile, edit the RTP endpoint with the IP address of the OpenVINO consumer application pod (to get IP address of the pod run: `kubectl exec -it openvino-cons-app ip a`)
* build the image for the [Client Simulator](#building-openvino-application-images)
